...
public class PDF_Generator_v2 {
	...
	private class TableOfContents
	{
		...
		private final int NUM_EACH_COL = 26;						// Number of recipes in each column of TOC
		private final int PAGE_WIDTH = 595;							// The width of the Page in pixels
		...
		public TableOfContents(Document document, PdfWriter writer, ContentsSection cs) throws Exception
		{
		    ...
		    if(allItems.length <= NUM_EACH_COL) {
		    	...
		    	int count = 0;
		    	double pageCount = 1;
		    	for(int k = 0; k < allItems.length - NUM_EACH_COL*2; k += NUM_EACH_COL, count += NUM_EACH_COL, pageCount += NUM_EACH_COL) {
		...
		private void fillPage(TableItem[] items, int startPos, double pageCount, int numLeft, int numRight) throws Exception
		{
			PdfContentByte cb = writer.getDirectContent();
