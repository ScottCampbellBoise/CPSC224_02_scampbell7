...
public class GlazeRecipe
{
			...
			if(glazeComponents != null)
			{
				if(glazeComponents.length == 1 && glazeComponents[0].getID() == comp.getID()) { 
					...
					for(int k = 0; k < glazeComponents.length; k++) { 
						...
						if(glazeComponents[k] != null) { size++; }
					}
		...
		public void addComponent(GlazeComponent newComponent)
		{
			if(glazeComponents == null)
			{
				...
				for(int k = 0; k < glazeComponents.length; k++) { 
					if(glazeComponents[k] != null && glazeComponents[k].getName().toLowerCase().trim().equals(newComponent.getName().toLowerCase().trim())
				...
				for(int k = 0; k < glazeComponents.length; k++) { 
					if(glazeComponents[k] != null && glazeComponents[k].getName().trim().toLowerCase().equals(newComponent.getName().trim().toLowerCase()) 
							&& glazeComponents[k].getAmount() != newComponent.getAmount()) 
					{
						isUpdate = true;
		...
		public void addAdd(GlazeComponent newComponent)
		{
			if(glazeAdds == null)
			{
				...
				boolean isDuplicate = false;
				for(int k = 0; k < glazeAdds.length; k++) { 
					if(glazeAdds[k] != null && glazeAdds[k].getName().toLowerCase().trim().equals(newComponent.getName().toLowerCase().trim())
				...
				boolean isUpdate = false;
				for(int k = 0; k < glazeAdds.length; k++) { 
					if(glazeAdds[k] != null && glazeAdds[k].getName().trim().toLowerCase().equals(newComponent.getName().trim().toLowerCase()) 
							&& glazeAdds[k].getAmount() != newComponent.getAmount()) 
					{
						isUpdate = true;
	        ...
	        try {
	            ...
	            if(glazeComponents != null) {
		            for(GlazeComponent gc : glazeComponents) { 
		            	if(gc != null) { compString += (gc.getName().trim() + " ; " + gc.getAmount() + " ~ "); }
		            }
	            ...
	            if(glazeAdds != null)
	            {
	            	for(GlazeComponent gc : glazeAdds) { 
	            		if(gc != null) { addString += (gc.getName().trim() + " ; " + gc.getAmount() + " ~ "); }
	            	}
		...
		private String getNewVersionString() {
			...
			if(lastSpaceIndex > 0 && newFileName.substring(lastSpaceIndex, newFileName.length()).contains("v")) // already has a v extension
			{
