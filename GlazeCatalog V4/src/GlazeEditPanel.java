	...
	private void updateAddsPanel(boolean addNewItem)
	{
		...
		if(adds == null) {
			...
			if(addNewItem) {
				...
				for(int k = 0; k < adds.length; k++) { if(adds[k] != null) { addPanel.add(adds[k]); } }
	...
	private void updateComponentsPanel(boolean addNewItem)
	{
		...
		if(components == null){
			...
			if(addNewItem) {
				...
				for(int k = 0; k < components.length; k++) { if(components[k] != null) {componentPanel.add(components[k]);} }
	...
	private class MessagePanel extends JPanel implements ActionListener
	{
		...
		public void displayMsg(String msg, int msgType)
		{
			...
			if(msgType == ERROR_MESSAGE) {
				messageField.setBackground(new Color(255, 179, 179, 100)); // Red Color
				addExitButton();
				isDisplayed = true;
				timer.start();
			} else if(msgType == WARNING_MESSAGE) {
				messageField.setBackground(new Color(255, 240, 179, 100)); // Yellow Color
				addExitButton();
				isDisplayed = true;
				timer.start();
			} else if(msgType == GENERAL_MESSAGE) {
				messageField.setBackground(new Color(125, 255, 104, 100)); // Green Color
				addExitButton();
				isDisplayed = true;
				timer.start();
			} else {
