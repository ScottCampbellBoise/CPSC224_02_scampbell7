...
public class GlazeSearchPanel extends JPanel
{
			...
			if(attr != null) {
				...
				if(info[0].contains("Color")) {
					for(int k = 0; k < resultSet.size(); k++) {
						if(!contains(info[1], resultSet.get(k).getColorAttribute())) { resultSet.remove(k); }
					}
					...
					if(!info[1].equals("Unknown")) {
						for(int k = 0; k < resultSet.size(); k++) {
							if(!contains(info[1], resultSet.get(k).getFiringAttribute())) { resultSet.remove(k); }
						}
					}
				}  else if (info[0].contains("Finish")) {
					if(!info[1].equals("Unknown")) {
						for(int k = 0; k < resultSet.size(); k++) {
							if(!contains(info[1], resultSet.get(k).getFinishAttribute())) { resultSet.remove(k); }
						}
					}
					...
					if(!info[1].equals("Unknown")) {
						for(int k = 0; k < resultSet.size(); k++) {
							if(!contains(info[1], resultSet.get(k).getFunctionalityAttribute())) { resultSet.remove(k); }
						}
					}
					...
					if(!info[1].equals("Unknown")) {
						for(int k = 0; k < resultSet.size(); k++) {
							if(!contains(info[1], resultSet.get(k).getCombinationAttribute())) { resultSet.remove(k); }
						}
					}
				}
			}
	...
	{
		for(String s : ary) {
			if(s.trim().equals(target.trim())) { return true; }
		}
		return false;
	}
	...
	public class GlazeAttribute extends JPanel
	{
		...
		{
			setBorder(BorderFactory.createCompoundBorder(new EmptyBorder(0,10,0,10), BorderFactory.createEtchedBorder()));
