...
public class GlazeGUI extends JFrame
{
	...
	private class MainPanel extends JPanel
	{
		...
		{
			for(GlazeRecipe gr : allRecipes) { if(gr.getName().equals(name)) { return gr; } }
			return null;
		}
		...
		{	
			...
			private String[] getClosestMatches(String text)
			{
				...
				String[] results;
				if(matches.length < 5) { results = new String[matches.length]; } else { results = new String[5]; }
			...
			private boolean compareArrays(String[] array1, String[] array2)
			{
				...
				for(int k = 0; k < array1.length && k < array2.length; k++) {
					if(!array1[k].equals(array2[k])) { return false; }
				}
			...
			private int compareNames(String s1, String s2) {
				...
				int sum = 0;
				for(int k = 0; k < s1.length() && numOff <= 2; k++) {
			...
			private class SearchResultPanel extends JPanel implements MouseListener
			{
				...
				public void mouseClicked(MouseEvent e) { openEditPanel(getRecipeByName(recipeName)); }
				public void mouseReleased(MouseEvent e) { }
				public void mouseEntered(MouseEvent e) { setBackground(highlightColor); }
			...
			private class MatchObj {
				private int match;
				private String name;
				public MatchObj(String name, int match) {this.name = name; this.match = match;}
				public String getName() {return name;}
				private int getMatch() {return match;}
			}
		}
		private class SuggestedGlazePanel extends JPanel
		{
			...
			public SuggestedGlazePanel()
			{
				...
				//Ensure that 5 recipes have been added. If not, then pad with random ones
				if(count < 4) {
					GlazeRecipe[] temp = new GlazeRecipe[5];
					int count2 = 0;
					for(int k=0; k<suggested.length; k++) { if(suggested[k] != null) { temp[count2] = suggested[k]; count2++; } }
					for(int k = count; k < temp.length; k++) { temp[k] = allRecipes[(int)(Math.random()*allRecipes.length)]; }
					suggested = temp;
				}
				createPanel();
			}
			
			// Returns the number of characteristics/attributes the recipes have in common
			private void createPanel()
			{
				TitledBorder border = new TitledBorder("Suggested Recipes"); border.setTitleFont(sectionFont);
			...
			private int getHighestMatchIndex(GlazeRecipe target, GlazeRecipe[] recipes)
			{
				...
				for(int k = 0; k < recipes.length; k++) {
					//Check if the recipe is one of the topRecipes or if it is allready in suggested
				...
				if(!highestFound) { return -1; } // Nothing found
				return maxPos;
			}
			private int compareGlazes(GlazeRecipe r1, GlazeRecipe r2)
			{
				int sum = 0;
				// make sure that if they are different versions of each other, then don't include
		...
		private class PdfGenerationPanel extends JPanel implements ActionListener
		{
			...
			public PdfGenerationPanel()
			{
				...
				try{ alpImage = uploadImage(new File("SampleAlph.png"));
				} catch(Exception e) { alpImage = null; }
				try{ typeImage = uploadImage(new File("SampleFiringType.png"));
				} catch(Exception e) { typeImage = null; }
				try{ rangeImage = uploadImage(new File("SampleFiringRange.png"));
				} catch(Exception e) { rangeImage = null; }
				try{ alphAndTypeImage = uploadImage(new File("SampleAlphAndFiringType.png"));
				} catch(Exception e) { alphAndTypeImage = null; }
				try{ alpAndRangeImage = uploadImage(new File("SampleAlphAndFiringRange.png"));
				} catch(Exception e) { alpAndRangeImage = null; }
				...
				imageLabel.setBorder(BorderFactory.createCompoundBorder(new EmptyBorder(10,10,10,10), BorderFactory.createTitledBorder("Sample Table of Contents")));
				
				saveButton = new JButton("Save Catalog to PDF");
				saveButton.addActionListener(this);
				
				JPanel buttonPanel = new JPanel();
				buttonPanel.setBorder(BorderFactory.createCompoundBorder(new EmptyBorder(10,10,10,10), BorderFactory.createTitledBorder("Recipe Arrangement Style")));
			    	...
			    	if(theFile.exists()) {
			    		...
			    		if (reply == JOptionPane.YES_OPTION) {
			    			...
			    			if(selectedOption.equals("A")) {
			    				isSaved = pdfGenerator.createCatalog(PDF_Generator_v2.ALPHABETICAL_ATTRIBUTE, theFile.getAbsolutePath());
							} else if (selectedOption.equals("Fr")) {
								isSaved = pdfGenerator.createCatalog(PDF_Generator_v2.CONE_ATTRIBUTE, theFile.getAbsolutePath());
							} else if (selectedOption.equals("Ft")) {
								isSaved = pdfGenerator.createCatalog(PDF_Generator_v2.FIRING_ATTRIBUTE, theFile.getAbsolutePath());
							} else if (selectedOption.equals("AFr")) {
								isSaved = pdfGenerator.createCatalog(PDF_Generator_v2.CONE_ATTRIBUTE, PDF_Generator_v2.ALPHABETICAL_ATTRIBUTE, theFile.getAbsolutePath());
							} else if (selectedOption.equals("AFt")) {
								isSaved = pdfGenerator.createCatalog(PDF_Generator_v2.FIRING_ATTRIBUTE, PDF_Generator_v2.ALPHABETICAL_ATTRIBUTE, theFile.getAbsolutePath());
							}
